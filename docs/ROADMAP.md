...
- [x] **Independent multi-strategy kernel + `xau_vwap_pullback` (shadow-only):** no winner-takes-all; news heat reserved before place; fair batch risk (News in the same tick); VWAP stop-pending; `live_ready` fail-closed on API/Telegram/Streamlit; pending heat restored after restart and harvest-before-overwrite so a fill between reconciles cannot drop reservation; News OCO reconstructed or leftover pending cancelled+verified after restart; comparison books isolate max_concurrent / daily DD / Three-Strikes; Telegram RequestException logs omit URL/token. Strategy stays `enabled: false` / `shadow_only: true` / `live_ready: false` — **not live-enabled**. Independent reviewer + security still required before merge. See `docs/STRATEGY_XAU_VWAP_PULLBACK.md`.
