...
- [x] **Honest override schema:** 11 `runtime_overrides` keys (`max_trades_*_day`, `cooldown_after_loss_minutes`, `max_portfolio_heat_pct`, weekly/monthly loss caps, `fail_closed_when_stale`, `single_instance`, slippage/overrun caps) were validated but never read by any code path. Listed in `config_overrides.UNENFORCED_OVERRIDE_KEYS`, marked `# INERT` in the example, and warned about at startup so they are not mistaken for risk controls
- [x] **Backtest/live parity disclosure:** `backtest.engine.LIVE_ONLY_GATES` records the nine entry guards the engine does not simulate; every summary carries `live_only_gates_not_modelled` so backtest trade counts are read as an upper bound, not a live forecast
