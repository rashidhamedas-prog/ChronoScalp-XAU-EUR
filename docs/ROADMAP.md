...
- [x] **Operator style engine (2026-09-04):** magic=0 gold bursts (7/14d, 86%
  *basket* WR, Iran 13:00–18:00) always had M15 ADX ≥ 22. Big SELL baskets
  faded RSI/Stoch spikes; BUY baskets were M5 pullbacks in an HTF uptrend.
  Encoded as a **standalone** `operator_style` engine (ADX + Stochastic +
  Iran 13:00–18:30 session), not bolted onto Delta. Gold catalog is Delta +
  operator style + news straddle; EUR is operator style + news straddle
  (failed EUR sweep-Delta stays off). Gold fade requires stoch **and** RSI;
  EUR fade may use stoch-only. Delta gold TP restored to 1.80R; operator
  style owns the 1.50R floor. One 1% ticket — no scale-in/grid/hedge.
