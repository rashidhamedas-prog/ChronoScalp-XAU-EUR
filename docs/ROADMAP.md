...
- [x] **Catalog slim (2026-08-31):** SMC, liquidity, news straddle, and VWAP removed from the live books after VPS logs showed they produced skip noise (Finnhub 403, low_rvol, spread_vs_median) and zero opens.
...
- [ ] EURUSD Delta redesign — 4 trades / −1.00R is an absence of edge, not a tuning problem. Investigate whether the entry filters adversely select the survivors before touching parameters. Until then the live overlay still lists EURUSD in Delta's `allowed_symbols`, so lifting the kill switch resumes unvalidated EURUSD risk.
