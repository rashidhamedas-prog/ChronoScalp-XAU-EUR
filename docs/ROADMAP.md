...
- [x] **Independent multi-strategy kernel + `xau_vwap_pullback` (shadow-only):** no winner-takes-all; news OCO cancels only the twin pending; DST sessions; Telegram simultaneous-OR + off/shadow/on for VWAP pullback. Strategy stays `enabled: false` / `shadow_only: true` — **not live-enabled**. See `docs/STRATEGY_XAU_VWAP_PULLBACK.md`. Independent reviewer + security still required before merge.
