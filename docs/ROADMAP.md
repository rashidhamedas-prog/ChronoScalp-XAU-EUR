...
- [ ] **In progress:** re-run both symbols on the parity engine and compare against the live journal rather than against the old backtest — the pre-parity numbers above cannot measure the trailing fix, since a bar-close model barely expresses the bug that caused the live bleed
