...
def _write_overrides(overrides_path: Path, payload: dict) -> None:
    ...
    validated = validate_runtime_overrides(payload if isinstance(payload, dict) else {})
    overrides_path.parent.mkdir(parents=True, exist_ok=True)
    overrides_path.write_text(
        yaml.safe_dump(validated, default_flow_style=False, allow_unicode=True, sort_keys=False),
        encoding="utf-8",
    )
...
def fetch_oanda_open_positions(
    ...
    """Fetch open OANDA positions for operator display."""
    if not api_token.strip() or not account_id.strip():
        return False, "اعتبارنامه OANDA ناقص است", []
    try:
        from chronoscalp.execution.oanda_broker import OANDABroker
        ...
        if not broker.connect():
            return False, "اتصال OANDA برای خواندن پوزیشن ناموفق بود", []
